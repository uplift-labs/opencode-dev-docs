# Conductor Orchestration Design

Design-only brief for a future OpenCode orchestration product. Recommended package name: `opencode-conductor`. Previous working name: `multitask`; this file keeps the old filename for link stability.
...
2. `conductor` plans independent work packages with file scopes, dependencies, models/agents, and risk labels.
3. For each work package, `conductor` chooses an execution mode: same-session subagent UX, same-workspace child session, new-workspace child session, or multiple sessions inside a new workspace.
4. After user approval, `conductor` creates only the sessions/workspaces required by that plan.
...
| Package | What Exists | Useful For `conductor` | Gap |
| --- | --- | --- | --- |
...
| Session identity fields | `packages/opencode/src/session/session.ts` | Session records include `title`, `parentID`, `metadata`, and `workspaceID`; use these to mark conductor-created workers. |
...
| Replacing `opencode-swarm` QA framework | `conductor` should orchestrate isolation-aware session/workspace fan-out, not replicate all guardrails. |
...
| Exposing raw OpenCode SDK as the model-facing API | Models should call stable `conductor_*` tools, not low-level `session.create`, `workspace.create`, or `promptAsync` primitives. |
...
| Bundled skill | Yes | Strategy layer: when to use `task`, when to call `conductor_start`, how to choose isolation mode, how to write worker prompts, and how to interpret reports. |
...
`conductor` should be a multi-surface package:

| Surface | Required? | Role |
| --- | --- | --- |
| Server plugin | Yes | Core orchestration, tools, state, events, workspace/session API calls, safety checks. |
| Skill | Yes | Model-facing operating manual for the `conductor_*` tools and isolation ladder. |
| TUI plugin | Optional | Command palette, dialogs, worker status panel, jump-to-worker actions. |
| Desktop/Web UI | No for v1 | Desktop users can still invoke server tools from chat; no custom visual surface. |
| External CLI | Optional later | Batch launch, CI integration, recovery/cleanup scripts. |
...
The model should learn `conductor` from two sources:

| Source | What The Model Sees | Purpose |
| --- | --- | --- |
| Plugin tool definitions | Tool names, descriptions, and argument schemas for `conductor_*`. | Gives the model safe executable actions. |
| Skill instructions | Decision ladder, examples, worker prompt templates, report contract, and when not to use `conductor`. | Teaches the model when and how to use those actions. |
...
```text
Model reads conductor skill -> chooses isolation mode -> calls conductor_start(plan) -> plugin executes SDK calls and stores state.
```
...
| --- | --- |
| `conductor_start` | Validates the approved plan, creates run state, creates child sessions/workspaces as needed, and dispatches prompts. |
| `conductor_status` | Reads durable run state plus live events/polling to report worker status. |
| `conductor_collect` | Parses report envelopes, validates worker identity, gathers diffs/tests, and produces a master summary. |
| `conductor_cancel` | Cancels selected worker sessions/background jobs and updates run state. |
| `conductor_cleanup` | Removes only approved completed workspaces/worktrees and optional audit exports. |
...
| --- | --- |
| Trigger conditions | Use `conductor` for multi-worker tasks that need coordination, status, report collection, or isolation escalation. |
...
| Tool usage | Show examples of `conductor_start`, `conductor_status`, and `conductor_collect` with valid argument shapes. |
...
| Safe tool contract | A skill cannot expose typed `conductor_start/status/collect` schemas; it can only describe what the model should do. |
...
| Constraints | No push, no merge to main, no modifying `.conductor/`; submit reports through the assigned worker report channel. |
...
```text
<CONDUCTOR_REPORT version="1">
{
  "runID": "cd-20260606-120000-auth",
  "workerID": "w01",
...
`conductor` should escalate isolation only when the cheaper mode is not enough.
...
| Create workspace | `experimental.workspace.create` with type `worktree`, branch `conductor/<run>/<worker-or-group>` or detached fallback. |
...
| Entity | Pattern |
| --- | --- |
| Run ID | `cd-YYYYMMDD-HHMMSS-shortgoal` |
| Worker ID | `w01`, `w02`, `w03` |
| Branch | `conductor/<run-id>/<worker-id>` |
| Session title | `[conductor <run-id> <worker-id>] <short task>` |
...
| --- | --- | --- |
| `conductor_start` | Master session only | Validate plan, create run, create workspaces/sessions, dispatch prompts. |
| `conductor_status` | Master or user | Show run and worker states. |
| `conductor_collect` | Master session | Aggregate final reports and diffs. |
| `conductor_cancel` | User/master | Stop selected workers and mark run cancelled. |
| `conductor_cleanup` | User/master | Remove completed workspaces/worktrees after merge or discard. |
| `conductor_merge_plan` | Master session | Produce safe integration order and conflict list; does not merge automatically in v1. |
...
| Worker sessions cannot call `conductor_start`. | Avoid runaway nested workspace creation. |
...
| --- | --- |
| Caller role | `conductor_*` tool handlers must verify `ctx.sessionID` or input `sessionID` against durable run state before doing lifecycle work. Do not rely on prompt instructions. |
...
| Project audit export | `.conductor/` in the master workspace | Optional human-readable export for reports and merge plans; auto-add to `.git/info/exclude` or ask before committing. |
| Worker reports | Runtime DB first; optional `.conductor/runs/<run-id>/reports/<worker-id>.md` export | Avoid treating each worker worktree as an independent source of truth. Master collection owns the export. |
| Event log | Runtime DB first; optional `.conductor/runs/<run-id>/events.jsonl` export | Append-only, useful for recovery/debug, but plugin state remains authoritative. |
...
4. When a worker becomes idle or a task result arrives, parse exactly one complete `<CONDUCTOR_REPORT version="1">...</CONDUCTOR_REPORT>` envelope from the final worker output.
...
7. Notify the master session through `conductor_status`/`conductor_collect` output, and optionally inject a concise synthetic status update if the implementation supports it safely.
...
| --- | --- |
| “Run this with conductor” | Master proposes worker plan and asks for approval. |
| “Status” | Master calls `conductor_status` and summarizes workers. |
| “Collect results” | Master calls `conductor_collect`, reads reports/diffs, produces merge plan. |
| “Cancel worker 2” | Master calls `conductor_cancel` for that worker. |
...
| TUI Feature | Purpose |
| --- | --- |
| Command palette `Conductor: Start` | Opens a prompt/dialog for goal and max workers. |
...
Use current TUI keymap API (`api.keymap.registerLayer`). TUI code should not assume OpenCode plugin tools are a general TUI-to-server RPC surface. For v1, prefer prompt-mediated UX: the TUI collects input and sends a master-session prompt that causes the model to call `conductor_start`. For zero-LLM direct start, the package must provide an explicit supported bridge, such as shared core code with a server-owned endpoint/action. TUI may read status through supported SDK/state paths, but it must not duplicate workspace/session creation logic or bypass server-side safety checks. Do not use deprecated `api.command.*`.
...
Conductor-created sessions should be visible through OpenCode's normal server/app state when they are created in the same Desktop sidecar/server. Workspace-mode work should be available through workspace APIs/status and `conductor_status`; do not promise that server-created workspaces automatically appear in every current Desktop sidebar/list without a verified app integration. Do not promise a custom Desktop panel in v1.

Important nuance: child sessions have `parentID`. The shared app's root session list filters for root sessions, so child sessions may appear as related/child task sessions, notifications, direct session links, or through `conductor_status`, but not necessarily as ordinary user-created root sessions in the sidebar.

User-facing identification requirements:

| Surface | Requirement |
| --- | --- |
| Session title | Prefix every worker title with `[conductor <runID> <workerID>]`, for example `[conductor cd-20260606-auth w02] update auth tests`. This is the fallback when metadata is not shown by UI. |
| Session metadata | Store `conductor: true`, `runID`, `workerID`, `role: "worker"`, `mode`, `masterSessionID`, optional `workspaceID`, and optional `branch`. |
| Parent linkage | Set `parentID` to the master session for worker child sessions whenever possible. |
| Workspace/branch names | Use `conductor/<run-id>/<worker-id>` or `conductor/<run-id>/<group-id>` for workspace-mode work. |
| Master output | `conductor_start` and `conductor_status` must list every worker with session ID, title, mode, status, workspace/branch when present, and a copyable/openable session reference if the client supports it. |
| Audit exports | Optional `.conductor/` exports should use the same run/worker IDs so Desktop-visible names and filesystem artifacts match. |

Never rely on the user manually distinguishing worker sessions by memory. If the UI only shows session titles, the title alone must be enough to tell that a session was created by conductor rather than manually by the user.
...
| State writes | Workers cannot modify `.conductor/`; master/plugin collection owns audit exports. |
| Recursive spawning | Workers cannot call `conductor_start`; master-only lifecycle tools. |
...
1. Should `conductor` be a standalone package or an `opencode-forge` contribution/mode?
...
6. Should `.conductor/` audit exports be generated automatically, only on demand, or disabled by default?
7. Should `conductor` integrate with existing `opencode-swarm`/`swarm-tools` file reservation formats, or keep its own minimal scope model?
...
| 1. Server MVP | Server plugin tools, durable state, and completion signaling. | `conductor_start/status/collect/cancel` work from chat and correctly parse worker reports. |
...
| 4. Safety MVP | Permission rules and hard enforcement. | Workers cannot edit outside assigned scope or start nested conductor runs. |
...
3. Confirm `same-session-subagent` workers have concrete child task/session IDs and return a complete `<CONDUCTOR_REPORT version="1">` envelope to the master session.
...
6. Confirm `conductor_status` reports running/done/blocked accurately across both non-workspace modes.
...
15. Confirm `conductor_collect` reads reports and produces a merge plan.
16. Confirm `conductor_cleanup` removes only selected completed workspaces, not same-workspace sessions.
...
Build `conductor` only if the exact isolation-aware fan-out/fan-in workflow is required and cannot be contributed to `opencode-forge`. The project should be intentionally thin: use OpenCode's existing subagent/session/workspace APIs, expose a small `conductor_*` tool surface, bundle a strategy skill, reuse proven patterns from Forge and Swarm, and focus on master-session orchestration plus report aggregation.
